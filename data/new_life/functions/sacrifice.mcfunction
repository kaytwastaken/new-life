...
# x=3671.5, y=65, z=308.5, distance=..3
execute if entity @p[x=3671.5, y=65, z=308.5, distance=..3, team=!dead] as @p run tag @s add location_good
# if no location_good
execute unless entity @p[x=3671.5, y=65, z=308.5, distance=..3, team=!dead] run tellraw @p {"text": "Your location lacks the necessary energy...", "color": "dark_gray", "italic": true}
...
# if totem_dropped
execute as @a[tag=location_good, tag=xp_good] if entity @e[type=minecraft:item, x=3671.5, y=65, z=308.5, distance=..3, nbt={Item:{id:"minecraft:shulker_shell", tag:{ Tags:[sacrificial_totem] } } } ] run tag @s add totem_good

# if no totem_dropped
execute as @a[tag=location_good, tag=xp_good] unless entity @e[type=minecraft:item, x=3671.5, y=65, z=308.5, distance=..3, nbt={Item:{id:"minecraft:shulker_shell", tag:{ Tags:[sacrificial_totem] } } } ] run tellraw @s {"text": "A Sacrificial Totem is needed...", "color": "dark_gray", "italic": true}
...
# visual
execute if entity @a[tag=sacrifice_finished] at @e[type=minecraft:item, x=3671.5, y=65, z=308.5, distance=..3, nbt={Item:{id:"minecraft:shulker_shell", tag:{ Tags:[sacrificial_totem] } } } ] run particle minecraft:flame ~ ~ ~ .4 .5 .4 .05 30 normal
execute at @a[tag=sacrifice_finished] run particle minecraft:totem_of_undying ~ ~1.5 ~ .8 1 .8 0 50 normal
execute as @a[tag=sacrifice_finished] run kill @e[type=minecraft:item, x=3671.5, y=65, z=308.5, distance=..3, nbt={Item:{id:"minecraft:shulker_shell", tag:{ Tags:[sacrificial_totem] } } } ]
